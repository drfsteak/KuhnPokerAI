...
class InformationSet():
    ...
    def normalize(self, strategy):
        ...
        else: # edge case creating an even probability distribution
    ...
    def get_strategy(self, reach_probability):
        ...
        self.strategy_sum += reach_probability
        return strategy
...
class KuhnPoker():
    ...
    def is_terminal(history): 
        return history in ['BC', 'BB', 'CC', 'CBC']
    
    def get_payoff(history, cards):
        if history in ['BC', 'CBC']: # opponent fold cases
            return +1
...
class KuhnPokerCFRTrainer():

    def __init__(self):
        self.infoset_map: Dict[str, InformationSet] = {}

    def get_information_set(self, card_and_history):
    ...
    def train(self, num_iterations):
        ...
        for _ in range(num_iterations):
            cards = random.sample(kuhn_cards)
            history = ''
            reach_probabilities = np.ones(2)
            util += self.cfr(cards,history,reach_probabilities, 0)
        return util
    
    def cfr(self, cards, history, reach_probabilities, active_player):
        if KuhnPoker.is_terminal(history):
            return KuhnPoker.get_payoff(history)
...
if __name__ == "__main__":
    ...
    cfr_trainer = KuhnPokerCFRTrainer()
